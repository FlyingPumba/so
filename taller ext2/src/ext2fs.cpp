...
{
	// cout << "Cargo inodo: " << inode_number << endl;
	// obtengo los datos del blockgroup
	unsigned int group_index = blockgroup_for_inode(inode_number);
	unsigned int group_offset = blockgroup_inode_index(inode_number);
	Ext2FSBlockGroupDescriptor *group_desc = block_group(group_index);

	// obtengo el inodo de la tabla de inodos del grupo
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int tabla_lba = group_desc->inode_table;

	unsigned int inodos_por_bloque = block_size / sizeof(Ext2FSInode);
	unsigned int block_address = tabla_lba + group_offset / inodos_por_bloque;
	unsigned int offset_in_block = group_offset % inodos_por_bloque;

	unsigned char buffer[block_size];
	read_block(block_address, buffer);
	Ext2FSInode *tabla = (Ext2FSInode*) buffer;

	// guardo el inodo en memoria
	Ext2FSInode *mem = (Ext2FSInode *)malloc(block_size);
	*mem = tabla[offset_in_block];
	return mem;
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	if (block_number < 12) {
		...
		block_number -= 12;
		// calculo cuantos punteros (entradas) hay en un bloque
		unsigned int block_size = 1024 << _superblock->log_block_size;
		unsigned int punteros_en_bloque_level_0 = block_size / sizeof(int);
		unsigned int punteros_en_bloque_level_1 = pow(punteros_en_bloque_level_0, 2);
		unsigned int punteros_en_bloque_level_2 = pow(punteros_en_bloque_level_0, 3);
...
unsigned int Ext2FS::get_block_address_aux(unsigned int level, unsigned int block_address, unsigned int block_number) {
	...
	if (level == 1) {
		...
		return get_block_address_aux(level-1, block_address, block_number);
	}
}
...
{
	cout << "Busco archivo: " << filename << endl;
	if(from == NULL)
	...
	unsigned int block_size = 1024 << _superblock->log_block_size;
	//unsigned int entradas_en_bloque = block_size / sizeof(Ext2FSDirEntry);

	// cargo todos los bloques en un buffer
	unsigned char buffer[(from->blocks)*block_size];
	for (block_number = 0; block_number < from->blocks; block_number++) {
		unsigned int block_address = get_block_address(from, block_number);
		read_block(block_address, buffer+block_number*block_size);
	}

	// recorro todos los DirEntry hasta que encuentro el archivo que estoy buscando
	unsigned char pos = 0;
	while(pos < from->size) {
		Ext2FSDirEntry * entrada = (Ext2FSDirEntry *)(buffer+pos);
		char *name = entrada->name;
		cout << "Encontré archivo: "<< name << endl;
		if(strcmp(filename, name) == 0) {
			// devuelvo el inodo del archivo
			// free(from);
			return load_inode(entrada->inode);
		} else {
			pos+= entrada->record_length;
		}
	}
	cout << "No encontré el archivo: " << filename << endl;
	return NULL;
}
...
int Ext2FS::read(fd_t filedesc, unsigned char * buffer, int size)
{
	...
	if(realsize > 0)
	{
		...
		// cout << "start: " << start_block << endl;
		// cout << "end: " << end_block << endl;
...
int Ext2FS::seek(fd_t filedesc, int offset)
{
	...
	if(position >= (int)_open_files[filedesc].size) {
